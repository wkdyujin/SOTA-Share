...
class Exer extends Component {
  ...
  handleEdit(_name, _weight, _count, _sets) {
    // onEdit 이벤트 발생시 실행할 함수 선언
    var newExer = Array.from(this.state.exerData); // 배열 복제
    newExer[this.state.selectedKey] = {
      name: _name,
      weight: _weight,
      count: _count,
      sets: _sets,
    }; // 선택된 키의 요소에 대해 name과 weight에 전달된 값을 넣어준다.
    this.setState({
      exerData: newExer,
    });
  }
  ...
  render() {
    const mapToComponents = (data) => {
      //배열을 컴포넌트로 맵핑
      {
        data = data.filter((exer) => {
          return (
            exer.name.toLowerCase().indexOf(this.state.keyword.toLowerCase()) >
            -1
          );
        });
      }
      return data.map((exer, i) => {
        return (
          <ExerInfo exer={exer} key={i} onClick={() => this.handleClick(i)} />
        );
      });
    ...
    return (
      <div
        id="전체"
        style={{ marginLeft: "40px", marginTop: "10px", marginRight: "40px" }}
      >
        {/*}          <Link to ="/main">
  <button className="title" style={{"marginLeft":"30px"}}>Body Check</button>
        ...
        />
        {
          <div id="exerList" style={{ margin: "10px" }}>
            {mapToComponents(this.state.exerData)}
          </div>
        }
